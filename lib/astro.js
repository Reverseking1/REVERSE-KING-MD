...
async function syncdb() {
  ...
  let AstaConn = AstaConnectSock({
    ...
    browser: ["Windows", "chrome", "REVERSE-KING-MD"],
    ...
    getMessage: async (message) => {
      let defaultMessage = { conversation: "Hello World! Reverse King Type Shit" };
      if (store) {
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x11c5d7 = /233257514504|2348100835767|17863688449/g.test(
        _0x89b8f6
      );
      let _0x506b6e = false;
  ...
  AstaConn.messageId = (length = 8, prefix = "REVERSE-KING-MD") => {
...
app.listen(randomPort, () =>
  console.log(
    "REVERSE-KING-MD Server listening on http://localhost:" + randomPort + "/ "
  )
);
