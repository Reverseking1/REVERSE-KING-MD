...
smd({
  pattern: "clear",
  fromMe: true,
  desc: "Delete WhatsApp chat history",
  type: "whatsapp",
}, async (message) => {
  try {
    // Ensure the message is valid and contains necessary info
    if (!message || !message.key || !message.jid) {
      return await message.send("No chat found to clear.");
    }

    // Modify the chat to delete the last messages in the chat
    await message.client.modifyChat(message.jid, "delete");

    // Confirmation message
    await message.send("_Chat cleared successfully!_");
  } catch (e) {
    // Send error message if the command fails
