...
smd(
  ...
  async (m) => {
    try {
      ...
      await m.send("..... you like Reverse King??🤔");
      ...
      const message = `*Response:* \n\n${resultText}\n\n*Powered by ReverseKing*`; // Add footer
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("𝑹𝑬𝑽𝑬𝑹𝑺𝑬 𝑲𝑰𝑵𝑮• ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            _0x2dfb59[0].includes("𝑹𝑬𝑽𝑬𝑹𝑺𝑬 𝑲𝑰𝑵𝑮 • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
...
smd(
  {
    pattern: "reverse",
    desc: "Get a response from 𝑹𝑬𝑽𝑬𝑹𝑺𝑬 𝑲𝑰𝑵𝑮 AI based on user query.",
    category: "ai",
    filename: __filename,
  },
  async (m) => {
    try {
      ...
      const query = m.text.split(' ').slice(1).join(' ');
      if (!query) {
        return await m.send("Please provide a query, e.g., `,ReverseKing What is the weather today?`.");
      }
      ...
      // Define the API URL
      const apiUrl = `https://api.giftedtech.my.id/api/ai/blackbox?q=${encodeURIComponent(query)}&apikey=king_Reverse-k7gjd8@gifted_api`;
